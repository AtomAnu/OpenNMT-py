...
def build_trainer(opt, device_id, model, fields, optim, model_saver=None, global_model=None, unsuper_reward=None):
    ...
    if opt.model_task not in [ModelTask.AC, ModelTask.A2C, ModelTask.A3C, ModelTask.PPO, ModelTask.ACSE]:
...
def prepare_special_tok_mask(tgt_field):

    tgt_bos = tgt_field.base_field.vocab.stoi[tgt_field.base_field.init_token]
    tgt_unk = tgt_field.base_field.vocab.stoi[tgt_field.base_field.unk_token]
    tgt_pad = tgt_field.base_field.vocab.stoi[tgt_field.base_field.pad_token]

    special_tok_mask = torch.ones(len(tgt_field.base_field.vocab)).to(torch.bool)

    special_tok_mask[tgt_bos] = ~special_tok_mask[tgt_bos]
    special_tok_mask[tgt_unk] = ~special_tok_mask[tgt_unk]
    special_tok_mask[tgt_pad] = ~special_tok_mask[tgt_pad]

    return special_tok_mask.to(torch.int64).to('cuda')
...
class SyncACTrainer(object):
    ...
    def _accum_batches(self, iterator):
        batches = []
        normalization = 0
        self.accum_count = self._accum_count(self.actor_optim.training_step)
        for batch in iterator:
            batches.append(batch)
            if self.norm_method == "tokens":
                num_tokens = batch.tgt[1:, :, 0].ne(
    ...
    def train(self,
        ...
        for i, (batches, normalization) in enumerate(
            ...
            report_stats = self._maybe_report_training(
                step, train_steps,
                self.actor_optim.learning_rate(),
                report_stats)
            ...
            if valid_iter is not None and step % valid_steps == 0:
                ...
                if self.gpu_verbose_level > 0:
                    logger.info('GpuRank %d: report stat step %d'
                                % (self.gpu_rank, step))
                self._report_step(self.actor_optim.learning_rate(),
                                  step, valid_stats=valid_stats)
                # Run patience mechanism
                if self.earlystopper is not None:
                    self.earlystopper(valid_stats, step)
    ...
    def validate(self, valid_iter, moving_average=None):
        ...
        if moving_average:
            ...
            for avg, param in zip(self.moving_average,
                                  valid_model.parameters()):
                model_params_data.append(param.data)
                param.data = avg.data.half() if self.actor_optim._fp16 == "legacy" \
                    else avg.data
...
class SyncACSETrainer(object):
    """
    Class that controls the training process for ACSE models.
    ...
    def __init__(self, model, train_loss, valid_loss, optim,
        ...
        self.train_loss = train_loss
        self.valid_loss = valid_loss
        # Using a single optimiser here because the encoder is shared
        self.optim = optim
        self.policy_strategy = policy_strategy
        self.policy_topk_sampling = policy_topk_sampling
        self.policy_sampling_temperature = policy_sampling_temperature
    ...
    def _gradient_accumulation(self, true_batches, normalization, total_stats,
        ...
        # TODO Update this so that the loss is computed correctly
            ...
            for j in range(0, target_size - 1, trunc_size):
                ...
                if self.accum_count == 1:
                    ...
                    if self.n_gpu > 1:
                        ...
                        onmt.utils.distributed.all_reduce_and_rescale_tensors(
                            grads, float(1))
                    gnorm = nn.utils.clip_grad_norm_(self.model.parameters(), 5.0)
                    self.optim.step()
...
class PPOTrainer(object):
    ...
    def train(self,
        ...
        for i, (batches, normalization) in enumerate(
                self._accum_batches(train_iter)):
            step = int((self.actor_optim.training_step - 1) / self.ppo_k_epochs) + 1
            # UPDATE DROPOUT
            self._maybe_update_dropout(step)
